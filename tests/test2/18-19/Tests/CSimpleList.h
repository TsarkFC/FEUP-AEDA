...
public:
	...
	{
        CNode *fList = first;
        CNode *sList = lst.first;

        while (fList != 0 || sList != 0){
            if (sList != 0){
                CNode* node = new CNode(sList->data(), fList->next());
	...
	int zipar() //Grupo 2 d)
	{
        ...
        while (fList != 0 && fList->next() != 0){
            //Checks if next element has same value
            if (fList->data() == fList->next()->data()){
                //Moves fList two times
                fList->setNext(fList->next()->next());
                //Increment variable to return
                counter++;
            }
