...
bool User::operator<(const User& u1) const {
    if (get<0>(titleReading) == "" && get<0>(u1.titleReading) == "" ) return titlesRead.size() < u1.titlesRead.size();
    else if (get<0>(titleReading) == "") return titlesRead.size() + 1 < u1.titlesRead.size();
    else if (get<0>(u1.titleReading) == "") return titlesRead.size() < u1.titlesRead.size() + 1;
    else return titlesRead.size() + 1 < u1.titlesRead.size() + 1;
}
bool User::operator==(const User& u1) const {
    if (get<0>(titleReading) == "" && get<0>(u1.titleReading) == "" ) return titlesRead.size() == u1.titlesRead.size();
    else if (get<0>(titleReading) == "") return titlesRead.size() + 1 == u1.titlesRead.size();
